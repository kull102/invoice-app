<template>
    <div @click="checkClick" ref="invoiceWrap" class="invoice-wrap flex flex-column">
      <form @submit.prevent="submitForm" class="invoice-content">
        ...
        <Loading v-show="loading" />
        <h1 v-if="!myStore.editInvoice">New Invoice</h1>
        ...
        <div class="save flex">
          ...
          <div class="right flex">
            <button v-if="!myStore.editInvoice" type="submit" @click="saveDraft" class="dark-purple">Save Draft</button>
            <button v-if="!myStore.editInvoice" type="submit" @click="publishInvoice" class="purple">Create Invoice</button>
            <button v-if="myStore.editInvoice" type="sumbit" class="purple">Update Invoice</button>
          </div>
        </div>
      </form>
    </div>
...
export default{
    ...
    created(){
      if(!this.myStore.editInvoice){
        this.invoiceDateUnix = Date.now()
        this.invoiceDate = new Date(this.invoiceDateUnix).toLocaleDateString('en-us',this.dateOptions)
      }
      if(this.myStore.editInvoice){
        const currentInvoice = this.myStore.currentInvoiceArray[0]
    ...
    methods:{
      checkClick(e){
        if(e.target === this.$refs.invoiceWrap){
          this.myStore.modalToggle()
        }
      },
      closeInvoice(){
        this.myStore.modalToggle()
        if(this.myStore.editInvoice){
          this.myStore.editToggle()
        }
      },
      addNewInvoiceItem(){
      ...
      async uploadInvoice(){
        ...
        this.loading = false
        this.myStore.invoiceToggle()
        this.myStore.getInvoices()
      },
      async updateInvoice(){
        if (this.invoiceItemList.length <= 0){
          alert('Please ensure you filled out work items!')
        ...
        this.myStore.updateInvoice(data)
      },
      submitForm(){
        if(this.myStore.editInvoice){
          this.updateInvoice()
